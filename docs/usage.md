...
Michaela otevře **Search** (`/search`) a zadá dotaz „deprecated". Výsledky jsou seskupeny podle entity type — vidí seznam služeb, C3 aplikací a capability položek, které obsahují slovo „deprecated" v názvu nebo statusu. Uloží tento search přes tlačítko „Save search", aby ho mohla přesnout kdykoli zopakovat. Klikne na každou nalezenou službu a v záložce Lifecycle ověří, zda je vyplněno pole `next_review_due_at`. Tři služby pole nemají — Michaela zapíše findings do svého audit reportu.
...
| Rychlé průřezové vyhledávání | Global Search se saved searches |
| Osobní přehled povinností | My Tasks s collapsible sekcemi |

---

*Dokument je součástí projektu s3c-manager. Poslední aktualizace: 2026-05-03.*
