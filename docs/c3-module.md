...
The old `/c3/capability-map` compatibility route is not part of the v1.2.2 final surface.
...
The current implementation deliberately keeps PDF storage/viewer out of v1.2.2 scope. Evidence document rows are generated from imported C3 taxonomy, spiral membership, and service mappings, and they intentionally do not contain developer-local PDF paths. If future FMN PDFs are imported as structured documents, they should be connected through the generic evidence model without reintroducing route-specific constants or route-specific dashboards.
