...
- The service detail, service editor and service graph use message keys (`service_detail.text.*`, `service_editor.text.*`, `service_graph.text.*`, cs/en) for about 560 texts instead of hardcoded strings translated in the DOM by `LegacyTextBridge`; translations are unchanged. About 50 texts in helper functions and module-level tables still go through the bridge.
