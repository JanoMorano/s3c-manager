...
- The service detail, service editor and service graph use message keys (`service_detail.text.*`, `service_editor.text.*`, `service_graph.text.*`, cs/en) for about 560 texts instead of hardcoded strings translated in the DOM by `LegacyTextBridge`; translations are unchanged. Helper functions and module-level label tables (studio tasks, link types, roles, C3 item types, detail views, counts, lifecycle labels) use keys too; counts use a neutral "Label: n" form instead of English plurals.
- The seven C3 link tables are one table, `c3_entity_link` (migration `42_c3_entity_link_table.sql`), with `link_kind` = graph edge kind and a check constraint per kind; uniqueness of capability links is kept by partial unique indexes. The old table names are auto-updatable views over it, so writers, imports, exports and the views built on them keep working; `v_c3_entity_link` reads the table directly. Link ids are renumbered by the migration.
- The reported schema version comes from the release (`middleware/src/config/schema-version.js`, checked by a test against the highest version registered by the schema files) instead of a hardcoded `2.2.1`; compose and `.env.example` no longer default `SCHEMA_VERSION`.
- `scripts/build-schema-baseline.sh` pins build-time timestamps, so rebuilding the baseline from the same files gives an identical dump.
- Frontend unit tests (Vitest, `npm test`) run in CI; the first cover graph highlighting, dependency layout, saved node positions and lifecycle stages. The i18n audit ignores test files.
...
### Fixed
- `/api/v1/admin/groups/<id>` with a non-numeric id returned 500 (database error); it now returns 400.
- Frontend lint warnings: the global search keeps results and suggestions per query instead of resetting state in effects and reads recent/saved searches through an external store; a missing hook dependency and an unused constant are fixed. Middleware lint knows Jest globals in colocated `*.test.js` files.
